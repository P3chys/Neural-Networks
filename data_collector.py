...
class StockDataCollector:
    ...
    def __get_data(self, ticker: str) -> pd.DataFrame:
        ...
        self.logger.log_info(f"Successfully collected data for {ticker}")
        return data


    def collect_daily_data(self) -> Dict[str, pd.DataFrame]:
        for ticker in self.default_tickers:
            self.logger.log_info(f"Collecting data for {ticker}")
            try:
                ...
                self.logger.log_error(f"Error collecting data for {ticker}: {str(e)}")
                continue
